...
const renderer: Partial<ReactRenderer> = {
  paragraph(children) {
    return <p className="text-sm leading-7 my-1">{children}</p>;
  },
  ...
  list(children, ordered) {
    return ordered ? (
      <ol className="list-decimal ml-8 my-2 text-sm">{children}</ol>
    ) : (
      <ul className="list-disc ml-8 my-2 text-sm">{children}</ul>
    );
  },
  listItem(children) {
    return <li className="my-1 leading-6">{children}</li>;
  },
  codespan(code) {
    return (
      <code className="px-2 py-0.5 text-xs rounded-md text-purple-800 dark:text-purple-300 bg-purple-600/20 font-mono">
        {code}
      </code>
    );
  },
  code(code, lang) {
    return (
      <pre className="my-3 rounded-lg bg-zinc-900 dark:bg-zinc-950 overflow-x-auto">
  ...
  tableHeader(children) {
    return (
      <thead className="text-xs font-medium uppercase bg-muted/50">{children}</thead>
    );
  },
  tableBody(children) {
  ...
  tableCell(children, flags) {
    return flags.header ? (
      <th className="p-3 text-sm font-medium">{children}</th>
    ) : (
