...
export default function SecuritySettings({ user }: { user: User }) {
  ...
  useEffect(() => {
    authClient.listAccounts().then(({ data }) => {
      if (data) setAccounts(data as { providerId: string }[]);
    });
  }, []);
  ...
  async function handleViewBackupCodes() {
    ...
    setTwoFaPending(true);
    try {
      const { data, error } = await (authClient.twoFactor as any).viewBackupCodes({ password: twoFaPassword });
        ...
        <Card className={!hasCredentials ? "opacity-50 pointer-events-none" : ""}>
          ...
          <CardContent>
            <form onSubmit={onEmailSubmit} className="space-y-4">
              ...
              <Button type="submit" disabled={emailPending} className="w-full sm:w-auto">
                {emailPending && <Loader2 className="size-4 mr-2 animate-spin" />}
                Send Verification
              </Button>
            </form>
          </CardContent>
          ...
          <CardContent>
            <form onSubmit={onPasswordSubmit} className="space-y-4">
              ...
              <Button type="submit" disabled={passwordPending} className="w-full sm:w-auto">
                {passwordPending && <Loader2 className="size-4 mr-2 animate-spin" />}
                Update Password
              </Button>
            </form>
          </CardContent>
            ...
            {twoFaStep === "idle" && (
              <div className="flex flex-wrap gap-2 pt-1">
                {!twoFaEnabled ? (
                  ...
                  <>
                    <Button variant="outline" onClick={handleViewBackupCodes} disabled={twoFaPending || !twoFaPassword}>
                      {twoFaPending ? <Loader2 className="size-4 mr-2 animate-spin" /> : <Eye className="size-4 mr-2" />}
                      View Backup Codes
                    </Button>
                    <Button variant="outline" onClick={handleRegenerateBackupCodes} disabled={twoFaPending || !twoFaPassword}>
                      {twoFaPending ? <Loader2 className="size-4 mr-2 animate-spin" /> : <RefreshCw className="size-4 mr-2" />}
                      Regenerate Codes
                    </Button>
                    <Button variant="destructive" onClick={handleDisable2FA} disabled={twoFaPending || !twoFaPassword}>
                      {twoFaPending && <Loader2 className="size-4 mr-2 animate-spin" />}
                      Disable 2FA
                    </Button>
                  </>
              ...
              <div className="space-y-4 rounded-lg border border-border bg-muted/30 p-4">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-2 rounded-md border border-border bg-background px-3 py-2">
                    <code className="flex-1 text-sm font-mono tracking-widest break-all">{totpSecret}</code>
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex gap-2">
                    ...
                    />
                    <Button onClick={handleVerifyTotp} disabled={twoFaPending || verifyCode.length !== 6}>
                      {twoFaPending ? <Loader2 className="size-4 animate-spin" /> : "Verify & Enable"}
                    </Button>
                  </div>
                </div>
                <Button variant="ghost" size="sm" onClick={() => { setTwoFaStep("idle"); setVerifyCode(""); }}>
                  Cancel
                </Button>
              </div>
            )}
              ...
              <div className="space-y-3 rounded-lg border border-border bg-muted/30 p-4">
                ...
                <div className="grid grid-cols-2 gap-2">
                  {backupCodes.map((code, i) => (
                    <code key={i} className="rounded-md bg-background border border-border px-3 py-1.5 text-sm font-mono text-center">
                      {code}
                    </code>
                  ))}
                </div>
